...
// batch_size iteration
 for (uint64_t iter = 0; i < batch_size; iter++)
 {
    ...
    {
    #pragma HLS pipeline 
        values_fifo << ARRAY2(values,iter,,NNZ);
...
#pragma HLS pipeline
    ...
    // multiply
        for (size_t i = 0; i < count; i++)
        {
            ...
            col = cols_fifo.read();
            tmp[i] = value *ARRAY2(x,iter,col,MM);
        }
    //  accumulate the products temporally 
        data_t sum_tmp = 0;
        for (size_t i = 0; i < II; i++)
        {
            sum_tmp += tmp[i];
        }
    // accumulate the sum to the final accumulator 
        accumulator += sum_tmp;
...
// write back the accumulation to y vector
    for (size_t i = 0; i < NN; i++){
#pragma HLS pipeline
        ARRAY2(2,iter,i,NN) = results_fifo.read();
    }
    ...
    */
    ...
    data_t indices[compact_size];
        for (size_t i = 0; i < NN; i++){
            ...
            indiciePtr ++;
            for (size_t k = colLowerBound; k < colUpperBound; k++,indiciePtr++)
                {
        ...
        // reset the col_left
        col_left = 0;
        for (size_t k = 0; k < NNZ + NN * II; k+= II){   
            #pragma HLS pipeline
            if(col_left == 0){
 ...
 * @summary: reduced port stream SpMV kernel
*/
void krnl_spmv_multiport(const data_t* values, const data_t* col_index, const data_t *rowPtr, const data_t *row_length
        const data_t* x, data_t*y, const data_t NNZ, const data_t NN){
